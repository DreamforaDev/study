...
object MovieDetailList {

    val movieDetailList = listOf(
        MovieDetail(
            1, "친척 집에서 구박받는 생활을 하던 해리는 11살 생일을 앞두고 호그와트 마법학교로부터 입학초대장을 받고 자신이 마법사라는 " +
                    "사실을 알게 된다. 해리는 호그와트 마법학교로 가는 열차에서 친구 론과 헤르미온느를 사귀고 함께 마법과 신비, " +
                    "모험으로 가득한 학교생활을 시작한다."
        ),

        MovieDetail(
            2, "도비는 해리에게 호그와트에 돌아가면 위험한 일을 당할 것이라 경고하며 그가 학교로 가는 것을 방해한다. " +
        ...
        MovieDetail(
            8, "해리와 론, 헤르미온느는 볼드모트를 불멸로 만들어주는 다섯 번째 호크룩스를 찾기 위해 호그와트로 돌아온다. " +
